...
const ControlButton = ({ onClick, children, className = "", ...props }) => {
  ...
  const handleKeyDown = (e) => {
    if (e.key === ' ' || e.key === 'Spacebar') { e.preventDefault(); e.stopPropagation(); }
  };
  return (
    <button onClick={onClick} onKeyDown={handleKeyDown} className={`${baseClasses} ${className || defaultClasses}`} {...props}>
      {children}
    </button>
  );
};
...
const PlayerTemplate = ({
  ...
  const getVolumeIcon = () => {
    ...
    if (volume < 0.7) return <SoundLowSolid width="24" height="24" />;
    return <SoundHighSolid width="24" height="24" />;
  };

  // prevent overlay buttons from toggling play
  const stop = (e) => { e.stopPropagation(); };

  return (
    <div
      ref={playerRef}
      className={`fixed top-0 left-0 w-screen h-screen bg-black ${showControls ? 'cursor-default' : 'cursor-none'}`}
    ...
    >
      ...
      {showControls && (
        <>
          {showSkipIntro && (
            <button
              onClick={(e) => { stop(e); onSkipIntro && onSkipIntro(); }}
              className="absolute top-6 right-6 z-40 px-4 py-2 rounded-md bg-white/90 text-black font-semibold shadow-lg hover:bg-white active:bg-gray-200 transition"
            >
              Skip Intro
            </button>
          )}
          {showSkipOutro && (
            <button
              onClick={(e) => { stop(e); onSkipOutro && onSkipOutro(); }}
              className="absolute top-6 right-6 z-40 mt-14 px-4 py-2 rounded-md bg-white/90 text-black font-semibold shadow-lg hover:bg-white active:bg-gray-200 transition"
            >
              {outroCountdown != null ? `Skip Outro · ${outroCountdown}s` : 'Skip Outro'}
            </button>
          )}
          {skipMarkersLoading && (
            <div className="absolute top-6 left-6 z-40 text-xs text-white/80 select-none">Loading timestamps…</div>
          )}
          {skipMarkersError && (
            <div className="absolute top-6 left-6 z-40 text-xs text-red-400 bg-black/40 px-2 py-1 rounded">{skipMarkersError}</div>
          )}
        </>
      )}
      ...
      {/* Controls bar */}
      <div
        className={`absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent pt-10 px-5 pb-2 transition-opacity duration-300 ease-in-out ${
          ...
          <div className="relative w-full h-1 bg-white/30 rounded-sm">
            ...
            <div
              className={`absolute top-1/2 -translate-y-1/2 w-4 h-4 bg-white rounded-full shadow-lg transition-all ${
                isProgressHovered || isDragging ? 'opacity-100 scale-100' : 'opacity-0 scale-75'
              }`}
        ...
        <div className="flex items-center justify-between text-white">
          <div className="flex items-center gap-2">
            <ControlButton onClick={onTogglePlay}>
              {isPlaying ? <PauseSolid width="28" height="28" /> : <PlaySolid width="28" height="28" />}
            </ControlButton>
            ...
            <div className="flex items-center gap-2 relative">
              ...
              <div
                className={`volume-slider-transition ${showVolumeSlider ? 'volume-slider-visible' : 'volume-slider-hidden'}`}
                onMouseEnter={onVolumeSliderMouseEnter}
                onMouseLeave={onVolumeSliderMouseLeave}
              >
                <div
                  ref={volumeSliderRef}
                  className="relative w-full py-3 my-4 cursor-pointer group"
                  onMouseDown={onVolumeSliderMouseDown}
                  onMouseEnter={onVolumeSliderHoverEnter}
                  onMouseLeave={onVolumeSliderHoverLeave}
                >
                  <div className="absolute inset-0 -my-2" />
                  <div className="relative w-full h-1 bg-white/30 rounded-sm">
                    ...
                    <div
                      className={`absolute top-1/2 -translate-y-1/2 w-3 h-3 bg-white rounded-full shadow-lg ${
                        isVolumeHovered || isVolumeDragging ? 'opacity-100 scale-100' : 'opacity-0 scale-75'
                      }`}
            ...
            <span className="text-sm text-white min-w-[140px] select-none">
          ...
          <div className="flex items-center gap-2">
            ...
            <ControlButton onClick={onToggleFullscreen}>
              {isFullscreen ? <Collapse width="24" height="24" /> : <Expand width="24" height="24" />}
            </ControlButton>
          </div>
        </div>
      </div>
